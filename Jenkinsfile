...
pipeline {
    ...
    parameters {
        string(name: 'ARCHIVE_LOCATION', defaultValue: 'Enter full path', description: 'Location of the Alias archive with the content for the installer')
        string(name: 'ARCHIVE_NAME', defaultValue: 'Alias-Build261-9d0c1d19-Win64-Qt5.12.zip', description: 'Name of Alias archive with the content for the installer')
        string(name: 'GIT_TAG_TO_USE', defaultValue: 'Build261', description: 'Alias version to package into installer')
        string(name: 'GIT_COMMIT_SHORT', defaultValue: '9d0c1d19', description: 'Short Git commit hash to package into installer')
    }
    stages {
        stage('Notification') {
            steps {
